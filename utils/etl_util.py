...
def aggregate_feature_with_groupby(df, config):
    """
    ...
    if not config:
        ...
        group_by = miscu.eval_elem_mapping(config, "group_by")
        aggre_gate = miscu.eval_elem_mapping(config, "agg")

        df_target = df.groupby(group_by).agg(aggre_gate).reset_index()
        return df_target


def aggregate_feature_with_pivot(df, config):
    """
    ETL feature to aggregate given dataframe.
    :param df: pd.DataFrame; Provided dataframe
    :param config: dict; Provided feature configuration
    ...
    if not config:
        ...
        df_target = df.pivot_table(index=group_by, aggfunc=aggre_gate).reset_index()
        ...
        for column_key, type_value in config.items():
            if column_key in df:
                ...
                # date type.
                    ...
                    df[column_key] = df[column_key].fillna(datetime.date(1970, 1, 1))
