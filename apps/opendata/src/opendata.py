...
#import apps.opendata.src.plugin_util
#import utils.etl_util as etlu
#import utils.misc_util as miscu

import frelabpy.utils.etl_util as etlu
import frelabpy.utils.misc_util as miscu
import frelabpy.apps.opendata.src.plugin_util
...
def run_transformation(args, config):
    ...
    #agg_config = miscu.eval_elem_mapping(config, 'aggregate')
    # aggregate feature performed using groupby function
    #df_target = etlu.aggregate_feature_with_groupby(df_target, agg_config)

    # aggregate feature performed using pivot function
    #df_target = etlu.aggregate_feature_with_pivot(df_target, agg_config)

    agg_config = miscu.eval_elem_mapping(config, 'aggregate')
    agg_method = args["Trans_method"]
    if agg_method == "groupby":
        # aggregate feature performed by groupby
        df_target = etlu.aggregate_feature_with_groupby(df_target, agg_config)
    elif agg_method == "pivot":
        # aggregate feature performed by pivot_table
        df_target = etlu.aggregate_feature_with_pivot(df_target, agg_config)
